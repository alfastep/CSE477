...
class Wumpus
{
    ...
    public function move($ndx) {
        $this->carried = false;
        return $this->moveTypes($ndx);
    }

    public function moveTypes($ndx){
        ...
        elseif($room->contains(self::PIT, 0)){
            return self::FELL;
        }

        elseif($room->contains(self::BIRDS, 0)){
            return $this->moveActual();
        }
    ...
    public function shoot($ndx) {
        ...
        if($room->contains(self::WUMPUS, 0)){
            ...
            if($room2->contains(self::WUMPUS, 0)){
                return true;
            }
    ...
    public function smellWumpus(){
        ...
        foreach($neighbors as $neighbor){
            if($neighbor->contains(self::WUMPUS, 0)){
                return true;
            }
        }

        foreach($neighbors as $neighbor){
            foreach($neighbor->getNeighbors() as $neighbors_neighbor){
                if($neighbors_neighbor->contains(self::WUMPUS,0)){
                    return true;
                }
            }
        }
    ...
    public function feelDraft(){
        ...
        $neighbors = $room->getNeighbors();
        foreach($neighbors as $neighbor){
            if($neighbor->contains(self::PIT,0)){
                return true;
            }
        }
    ...
    public function hearBirds(){
        ...
        $neighbors = $room->getNeighbors();
        foreach($neighbors as $neighbor){
            if($neighbor->contains(self::BIRDS,0)){
                return true;
            }
        }
